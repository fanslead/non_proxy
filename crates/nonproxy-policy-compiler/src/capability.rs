...
use nonproxy_model::{DecisionSpec, IpFamily, OutboundId, Policy, RouteAction, Transport};
use nonproxy_policy::OutboundCapabilities;

use crate::PolicyConflict;
...
impl CompileCapabilities {
    ...
    fn validate_decision(
        ...
        let Some(outbound_id) = decision.outbound_id() else {
            return;
        };
        let Some(outbound) = self.outbounds.get(outbound_id).copied() else {
        ...
        if required_transports(self, policy)
            .into_iter()
            .any(|transport| !outbound.supports_transport(transport))
        {
            push_decision_conflict(
                conflicts,
                policy,
        ...
        if required_families(self, policy)
            .into_iter()
            .any(|family| !outbound.supports_family(family))
        {
            push_decision_conflict(
                conflicts,
                policy,
