...
pub(crate) struct PersistedDecision {
    ...
    pub(crate) app_signer_id: Option<String>,
    pub(crate) app_parent_stable_id: Option<String>,
    pub(crate) app_helper_group_id: Option<String>,
...
pub(crate) fn persisted(
    ...
    Ok(PersistedDecision {
        ...
        app_signer_id: input.app.signer_id().map(str::to_owned),
        app_parent_stable_id: input.app.parent_stable_id().map(str::to_owned),
        app_helper_group_id: input.app.helper_group_id().map(str::to_owned),
    ...
    connection
        .query_row(
            "SELECT occurred_at_unix_ms, snapshot_version, app_stable_id,
                    app_display_name, app_platform, app_signer_id,
                    app_parent_stable_id, app_helper_group_id, destination_redacted,
...
fn decode_persisted_at(
    ...
    Ok(PersistedDecision {
        ...
        app_signer_id: row.get(start + 5)?,
        app_parent_stable_id: row.get(start + 6)?,
        app_helper_group_id: row.get(start + 7)?,
        destination_redacted: row.get(start + 8)?,
        transport: parse_transport(row.get(start + 9)?).map_err(storage_to_sql)?,
        destination_port: decode_u16(row, start + 10, "connection_decision.destination_port")?,
        matched_policy_id: row.get(start + 11)?,
        matched_rule_id: row.get(start + 12)?,
        action: parse_action(row.get(start + 13)?).map_err(storage_to_sql)?,
        failure_mode: parse_failure_mode(row.get(start + 14)?).map_err(storage_to_sql)?,
        reason_code: row.get(start + 15)?,
        evidence_level: EvidenceLevel::parse(row.get(start + 16)?).map_err(storage_to_sql)?,
        interface_name: row.get(start + 17)?,
        outbound_id: row.get(start + 18)?,
        exit_probe_id: row.get(start + 19)?,
        fail_open_direct: decode_bool(row, start + 20, "connection_decision.fail_open_direct")?,
        decision_latency_micros: decode_optional_u64(
            row,
            start + 21,
            "connection_decision.decision_latency_us",
        )?,
        error_code: row.get(start + 22)?,
        provider_id: row.get(start + 23)?,
        provider_generation: decode_u64(
            row,
            start + 24,
            "connection_decision.provider_generation",
        )?,
        flow_id: row.get(start + 25)?,
    })
}
