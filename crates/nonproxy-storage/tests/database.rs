...
fn file_database_migrates_once_and_backs_up_metadata() {
    ...
    assert_eq!(database.migration_report().current_version(), 6);
    assert_eq!(database.migration_report().applied().len(), 6);
    ...
    assert_eq!(reopened.migration_report().previous_version(), 6);
