...
fn file_database_migrates_once_and_backs_up_metadata() {
    ...
    assert_eq!(database.migration_report().current_version(), 1);
    assert_eq!(database.migration_report().applied().len(), 1);
    ...
    assert_eq!(reopened.migration_report().previous_version(), 1);
