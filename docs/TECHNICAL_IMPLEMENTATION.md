...
出口组的权威持久化使用初始 schema 中的 `outbound_group` 与
...
`ProxyTarget::Group`，因此同名标识也不会混淆；仓储读取会拒绝同时存在两种目标的损坏数据，
...
后台调度器每 30 秒从 SQLite 权威状态读取最多 100 个到期且启用的源，同一源只允许一个在途
任务，最多并发 4 个。调度始终复用上述服务，不直接接触 URL 或数据库写路径；一个源失败不会
中止其他任务。手动 RPC 或客户端连接被取消后，已启动的原子刷新继续完成。Unix/Windows 关机
...
共享 Avalonia 桌面端把出口组作为“网络出口”中的独立子模块，不新增平台专属配置页。
普通界面不暴露策略表达式或内部组 ID：新组 ID 由客户端生成，用户只填写名称，从已有出口中
选择成员并通过纵向线路栈调整顺序。候选列表只包含 `SupportsDefaultRoute` 的完整流量出口，
保存仍由服务端重新校验 2～32 个不重复成员、成员存在性和能力。删除必须二次确认，且只删除组，
不删除成员出口。

桌面端并行读取出口目录与组目录，并核对相同的 `routing_revision` 和默认组身份；目录在读取期间
发生变化时重试一次，持续不一致则提示刷新，不拼接两个时点的状态。创建和编辑使用组 revision
CAS；编辑当前默认组必须收到 `PENDING_ACK` 快照才视为已接受。将组设为默认后，单出口默认标记
立即清除，但文案只声明配置已保存和等待确认。首版限时“全部代理”覆盖仍只接受单出口；当常规
默认路由是组时，该按钮保持禁用并解释边界，不能把组 ID 当成单出口 ID 发送。
