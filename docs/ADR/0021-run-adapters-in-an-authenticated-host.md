...
7. `apply` 通过协调事务应用专属 sidecar 与主配置，再通过客户端公开入口重载；重载或确认
   失败时自动恢复两个文件并重新载入备份，完整演进见 ADR-0026。`verify` 仍最多返回
   `EVIDENCE_LEVEL_CONFIGURATION`，顶层 `verified` 与 `path_verified` 固定为 false。
   桌面端不得把配置或重载成功显示为“已接管”或“已经直连”。

## 当前边界

- 已接入第三方主配置、双文件回滚、公开重载和失败自动恢复；尚未接入实际决策/出口路径
  验证。完整演进见 ADR-0025 与 ADR-0026。
- Windows 已复用 Protobuf、目录、事务语义和认证命名管道；按用户签名分发、启动/升级以及
  状态文件精确 DACL 已有源码，x64 CI 运行 ACL 与原子文件事务测试，ARM64 保留交叉编译门禁。
  真实登录任务、多用户隔离、命名管道 RPC 和第三方客户端重载仍需系统验收，源码与 CI
  结果不代表产品已经可用。
- NonProxy 不捆绑、下载或托管 Mihomo/sing-box 核心；这里只检测用户明确选择的既有程序，
  并通过明确登记主配置中的公开控制入口执行预检和重载。

## 后果

- 第三方客户端崩溃、挂起和超量输出不会拖住 UI 或 `gatewayd`。
- 本机调用方必须同时拥有私有 UDS/命名管道访问权和当前会话能力，重复 RPC 保持幂等。
