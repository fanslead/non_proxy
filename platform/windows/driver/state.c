    ...
    _Out_ NP_WFP_DEVICE_EXTENSION* Extension)
{
    ...
    Extension->Config.Size = sizeof(NP_WFP_CONFIG_V1);
}
    ...
    _In_ const NP_WFP_CONFIG_V1* Config)
{
    ...
    if (Config->Magic != NP_WFP_CONFIG_MAGIC ||
        ...
        Config->Size != sizeof(NP_WFP_CONFIG_V1) ||
        (Config->Flags & ~NP_WFP_CONFIG_FLAG_ENABLED) != 0) {
        return STATUS_REVISION_MISMATCH;
    }
    if ((Config->Flags & NP_WFP_CONFIG_FLAG_ENABLED) != 0 &&
        (Config->ProxyProcessId == 0 ||
         Config->ProxyProcessId > MAXULONG ||
...
{
    NP_WFP_CONFIG_V1 config = NonProxyReadConfig(Extension);
