...
已经落地的基础包括版本化契约、Rust 策略模型/编译器/SQLite 权威存储、认证 UDS 控制面、Avalonia 共享产品工作区，以及 macOS Swift Provider 的快照校验、缓存、纯函数决策和 Provider RPC 客户端。Transparent Proxy 已具备真实 Provider 入口、来源签名身份、TCP/UDP 目标解析、DIRECT/BLOCK 与显式失败模式；代理 flow relay、DNS Proxy、浏览器学习与 Windows WFP 仍按里程碑继续实现。尚未启用的数据路径和 RPC 会明确失败，不伪造成功。
