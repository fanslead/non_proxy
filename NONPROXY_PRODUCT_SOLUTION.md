...
目标认证测试。远程订阅 URL/Token 已进入系统凭据库，并具备安全公网获取、原子节点刷新、
后台调度、持久凭据回收和共享桌面管理。协议范围只包括六种 AEAD/AEAD-2022 方法，不包括
SIP003 plugin、VMess/VLESS、Trojan、Hysteria 2、TUIC、WireGuard、OpenVPN 或 OpenConnect；
新增协议仍须按独立协议核心、许可证审查和系统验收推进。
...
Windows 使用 WFP `FwpmGetAppIdFromFileName0` 产生的 ALE App ID 作为稳定身份，并用
...
“让此应用始终直连”。macOS Helper 记录按父应用稳定身份建规则，并默认包含同组辅助进程；
Windows 使用精确可执行文件规则。旧记录、
未签名/未知应用、其他平台记录和已有应用规则只显示原因，不能按显示名猜测或创建规则。
用户必须在确认卡中核对应用与签名身份；保存成功但 Provider 尚未 ACK 时只显示“等待系统
组件确认”，不能提前声称已经直连。
...
5. DNS 走直连解析路径。
6. 其他应用仍走 PROXY。
7. 身份保持稳定的正常应用更新后规则仍然有效；改变 Windows 安装路径或包身份的更新必须
   重新选择，不能静默扩大到同名文件。
