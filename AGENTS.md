...
- `gatewayd` 升级不得只信任 `SMAppService.status`；必须校验由已签名二进制和 LaunchAgent 模板生成的包指纹及私有运行身份。替换后台服务前先撤销网络偏好，等待旧进程退出，确认新版本就绪后才能恢复网络接管。
...
- 候选确认只能针对已结束站点会话的真实候选，必须包含主站；规则批次、逐候选决策和确认收据须原子写入。相同确认 ID 只能重放相同会话和完全相同的域名集合，快照事务中断必须可恢复。
...
- 停止和到期会话必须进入对应标签页的候选审核；待审核期间不得继续持有全站读取权限。标签页关闭必须丢弃其审核上下文，不能按域名猜测关联到其他标签页。
...
- 所有人工编写的代码注释必须使用中文并采用 UTF-8；第三方代码和生成文件除外。
- 所有人工维护的文本文件必须使用 UTF-8；二进制资产不适用此规则。
